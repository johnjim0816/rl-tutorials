...
class Launcher:
    ...
    def test_one_episode(self,i_ep, env, agent, cfg, logger):
        ep_reward = 0
        ep_step = 0
        return agent,ep_reward,ep_step
    ...
    def run(self):
        ...
        if cfg.mode.lower() == 'train':
            ...
            for i_ep in range(cfg.train_eps):
                agent,ep_reward,ep_step = self.train_one_episode(i_ep, env, agent, cfg, logger)
            ...
            for i_ep in range(cfg.test_eps):
                agent,ep_reward,ep_step = self.test_one_episode(i_ep, env, agent, cfg, logger)
