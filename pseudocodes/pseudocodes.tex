	...
	\begin{algorithmic}[1] % [1]显示步数
		...
		\FOR {回合数 = $1,2,\cdots,M$}
			...
			\FOR {epoch数 = $1,2,\cdots,K$}
				...
				\STATE 通过最大化目标函数$L^{CLIP}(\theta)$更新Actor参数，$\theta_{m+1} \leftarrow \theta_{m}$：
				\STATE $L^{CLIP}(\theta)= \frac{1}{|D_m|T}\sum_{\tau \in D_m}\sum_{t=0}^{T} min(\frac{\pi_\theta(a_t|s_t)}{\pi_{\theta_{m}}(a_t|s_t)} A^{\pi_{\theta_{m}}}(s_t,a_t), g(\epsilon,A^{\pi_{\theta_{m}}}(s_t,a_t)))$\footnotemark[3]
