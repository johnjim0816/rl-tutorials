...
class RainbowModel(nn.Module):
    def __init__(self, input_dim,output_dim,hidden_dim=128):
        ...
        self.noisy_value3 = NoisyLinear(hidden_dim, 1)

        self.noisy_advantage2 = NoisyLinear(hidden_dim, hidden_dim) # NoisyDQN + Dueling DQN
        self.noisy_advantage3 = NoisyLinear(hidden_dim, output_dim)
        
    def forward(self, x):
        x = F.relu(self.fc1(x))

        value = F.relu(self.noisy_value2(x))
        value = self.noisy_value3(value)

        advantage = F.relu(self.noisy_advantage2(x))
        advantage = self.noisy_advantage3(advantage)
...
class Agent:
    def __init__(self, cfg) -> None:
        ...
        self.target_net = RainbowModel(cfg.n_states,cfg.n_actions,hidden_dim=cfg.hidden_dim).to(self.device)
    ...
    def update(self):
        ...
        ## update the importance value in the PER_DQN 
